...
static void ECP_free_pubkey_content(struct pubkey_content *pkc,
				    const struct logger *logger)
{
...
static bool ECP_pubkey_same(const struct pubkey_content *lhs,
...
static size_t ECP_strength_in_bits(const struct pubkey *pubkey)
{
...
const struct pubkey_type pubkey_type_ecdsa = {
	...
	.ipseckey_algorithm = IPSECKEY_ALGORITHM_ECDSA,
	.ipseckey_rdata_to_pubkey_content = ECP_ipseckey_rdata_to_pubkey_content,
