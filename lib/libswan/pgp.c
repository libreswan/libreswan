...
static size_t pgp_size(chunk_t *blob, int len)
{
	size_t size = 0;
...
static bool parse_pgp_pubkey_packet(chunk_t *packet, pgpcert_t *cert)
{
	...
	if (version == 3) {
		...
		DBG(DBG_PARSING,
		    ...
		    DBG_log("L3 - until:");
		    DBG_log("  %s",
			    timetoa(&cert->until, TRUE, tbuf, sizeof(tbuf))));
	}
	...
	case PGP_PUBKEY_ALG_RSA_SIGN_ONLY:
		...
		/* public exponent e */
		cert->publicExponent.len =
			(pgp_size(packet, 2) + 7) / BITS_PER_BYTE;
		...
		if (version == 3) {
			...
			MD5_CTX context;
			osMD5Init(&context);
			osMD5Update(&context, cert->modulus.ptr,
				    cert->modulus.len);
			osMD5Update(&context, cert->publicExponent.ptr,
				    cert->publicExponent.len);
			osMD5Final((unsigned char *)cert->fingerprint,
				   &context);
		} else {
			libreswan_log(
				"  computation of V4 key ID not implemented yet");
		}
		break;
 ...
 */
static bool parse_pgp_secretkey_packet(chunk_t *packet, rsa_privkey_t *key)
{
	int i;
	int s2k;
...
bool parse_pgp(chunk_t blob, pgpcert_t *cert, rsa_privkey_t *key)
{
	DBG(DBG_PARSING,
	    DBG_log("L0 - PGP file:")
	    );
		...
		if (packet_tag & 0x40) {
			...
			DBG(DBG_PARSING,
			    DBG_log("  %s (%d), old format, %d bytes",
				    ...
				    "Undefined Packet Type", packet_type,
				    (int)packet.len);
			    DBG_log("L2 - body:")
			    );
			...
			if (cert != NULL) {
				...
				case PGP_PKT_PUBLIC_KEY:
					if (!parse_pgp_pubkey_packet(&packet,
								     cert))
						return FALSE;

					break;
				case PGP_PKT_SIGNATURE:
					if (!parse_pgp_signature_packet(&packet,
									cert))
						return FALSE;
				...
				case PGP_PKT_SECRET_KEY:
					if (!parse_pgp_secretkey_packet(&packet,
									key))
						return FALSE;
...
static bool same_pgpcert(pgpcert_t *a, pgpcert_t *b)
{
	return a->certificate.len == b->certificate.len &&
	       memcmp(a->certificate.ptr, b->certificate.ptr,
		      b->certificate.len) == 0;
}
 ...
 */
pgpcert_t*add_pgpcert(pgpcert_t **ppgpcerts, pgpcert_t *cert)
{
 ...
 */
void release_pgpcert(pgpcert_t **ppgpcerts, pgpcert_t *cert)
{
