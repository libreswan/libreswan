#!/usr/bin/python
""" dist_certs.py: create a suite of x509 certificates for the libreswan test harness
...
import os, ssl, time, shutil, subprocess, sys
...
def reset_files():
	for dir in ['keys/', 'cacerts/', 'certs/', 'pkcs12/', 'pkcs12/curveca', 'crls/']:
...
def writeout_privkey(filename, item, type=crypto.FILETYPE_PEM):
	with open(filename, "w") as f:
...
def create_csr(pkey, CN, C=None, ST=None, L=None, O=None, OU=None, emailAddress=None, algo='sha1'):
	""" Create the certreq """
...
def set_cert_extensions(cert, issuer, isCA=False, isRoot=False):
	...
	'URI:ldap://nic.testing.libreswan.org/o=Libreswan, c=CA?certificateRevocationList?base?(objectClass=certificationAuthority)')])
	...
	cert.add_extensions([crypto.X509Extension('basicConstraints', False, bc)])

	if isRoot:
		skisub = issuer
	else:
		skisub = cert
		cert.add_extensions([crypto.X509Extension('keyUsage', False, 'digitalSignature,nonRepudiation,keyEncipherment')])
		cert.add_extensions([crypto.X509Extension('extendedKeyUsage', False, 'serverAuth,clientAuth,codeSigning')])

	cert.add_extensions([crypto.X509Extension('subjectKeyIdentifier', False, 'hash', subject=skisub)])
	cert.add_extensions([crypto.X509Extension('authorityKeyIdentifier', False, 'keyid:always,issuer:always', issuer=issuer)])
	cert.add_extensions([crypto.X509Extension('crlDistributionPoints', False, CRL_URI)])

def create_sub_cert(CN, CACert, CAkey, snum, START, END,
					C='ca', ST='Ontario', L='Toronto', O='Libreswan', OU='Test Department', emailAddress='testing@libreswan.org',
					ty=crypto.TYPE_RSA, keybits=1024, sign_alg='sha1', isCA=False):
...
def create_root_ca(CN, START, END,
				   C='ca', ST='Ontario', L='Toronto', O='Libreswan', OU='Test Department', emailAddress='testing@libreswan.org',
				   ty=crypto.TYPE_RSA, keybits=1024, sign_alg='sha1'):
	...
	set_cert_extensions(cacert, cacert, isCA=True, isRoot=True)
...
def gen_gmtime_dates():
	...
	return dict(OK_NOW=gmc(ok_stamp), OLD=gmc(two_days_ago_stamp), OLD_END=gmc(two_days_ago_end_stamp),
			    FUTURE=gmc(future_stamp), FUTURE_END=gmc(future_end_stamp))

def store_cert_and_key(name, cert, key):
	""" Places a ca or end cert and key in the script's globals """
...
def create_basic_pluto_cas(ca_names):
	...
	for name in ca_names:
		...
		ca, key = create_root_ca(CN="Libreswan test CA for " + name, START=dates['OK_NOW'], END=dates['FUTURE'])
...
def create_mainca_end_certs(mainca_end_certs):
	...
	for name in mainca_end_certs:
		...
		if name == 'unwisechar':
			common_name = 'unwisechar ~!@#$%^&*()-_=+;:/?<>.testing.libreswan.org'
		elif name == 'spaceincn':
		...
		cert, key = create_sub_cert(common_name, ca_certs[signer][0], ca_certs[signer][1],
									serial, O=org, START=startdate, END=enddate, keybits=keysize, sign_alg=alg)
...
def create_chained_certs(chain_ca_roots):
	""" Create the EE->IA1->IA2->IAx-->CA chains. Last in the chain is the end cert
...
def create_leading_zero_crl():
	...
	while True:
		...
		crl = zerosig.export(signcert, signkey, type=crypto.FILETYPE_TEXT, days=days)
		pem = zerosig.export(signcert, signkey, type=crypto.FILETYPE_PEM, days=days)
		...
		if good:
			print nl
			print "found after %d signatures!" % (days) # fun statistic from this attack. watch md5 sigs go after 38~ tries. 
...
def create_crlsets():
	...
	revoked.set_serial(str(end_certs['revoked'][0].get_serial_number()))

	validcrl = crypto.CRL()
	validcrl.add_revoked(revoked)
	with open("crls/cacrlvalid.pem", "wb") as f:
		f.write(validcrl.export(ca_certs['mainca'][0], ca_certs['mainca'][1], days=15))

	# the old dist_certs had identical, empty crls for "notyetvalid" and "crlexpired". previously useless?
	# they will probably need to be created by
	# openssl ca -gencrl -startdate $START -enddate $END -out crls/cacrlexpired.pem
	# -keyfile keys/mainca.key -cert cacerts/mainca.crt -passin pass:foobar

	expired = crypto.CRL()
	with open("crls/cacrlexpired.pem", "wb") as f:
		#days=0 works. Should expired have a revoked cert?
		f.write(expired.export(ca_certs['mainca'][0], ca_certs['mainca'][1], days=0))

	notyet = crypto.CRL()
	with open("crls/cacrlnotyetvalid.pem", "wb") as f:
		f.write(notyet.export(ca_certs['mainca'][0], ca_certs['mainca'][1], days=15))

	othercrl = crypto.CRL()
	with open("crls/othercacrl.pem", "wb") as f:
		f.write(othercrl.export(ca_certs['otherca'][0], ca_certs['otherca'][1], days=15))
...
def create_ec_certs():
	...
	#create CA
	pexpect.run('openssl ecparam -out keys/curveca.key -name secp384r1 -genkey -noout')
	child = pexpect.spawn('openssl req -x509 -new -key keys/curveca.key -out cacerts/curveca.crt -days 3650 -set_serial 1')
	...
	for name in ['east', 'west', 'north', 'road']:
		...
		#create end certs
		pexpect.run('openssl ecparam -out keys/' + name + '-ec.key -name secp384r1 -genkey -noout')
		child = pexpect.spawn('openssl req -x509 -new -key keys/curveca.key '
							  '-out certs/' + name + '-ec.crt -days 365 -set_serial ' + str(serial))
		child.expect('Country Name')
		child.sendline('ca')
		child.expect('State')
		child.sendline('Ontario')
		...
		serial = serial + 1
		#package p12
		pexpect.run('openssl pkcs12 -export -inkey keys/%s-ec.key -in certs/%s-ec.crt -name %s-ec'
					' -certfile cacerts/curveca.crt -caname "curveca" -out pkcs12/curveca/%s-ec.p12'
					' -passin pass:foobar -passout pass:foobar' % (name, name, name, name))

def run_dist_certs():
	""" Generate the pluto test harness x509 certificates, p12 files, keys, and CRLs """

	# Add root CAs here
	basic_pluto_cas =  ('mainca', 'otherca', 'east_chain_root', 'west_chain_root')

	# Add end certs here
	mainca_end_certs = ('east','west','sunset','sunrise','north','south',
						'pole','park','beet','carrot','nic','japan','bigkey',
						'notyetvalid','notvalidanymore','signedbyother','wrongdnorg',
						'unwisechar','spaceincn','hashsha2','cnofca','revoked')
