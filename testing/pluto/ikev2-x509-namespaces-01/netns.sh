...
# Setting route on the host results in a default route that does not actually work,
# so we run this command on the host for all network spaces in roadinit.sh. Weird..
# But we need the default route for pluto to load the connection, so add and delete :/
ip netns exec $name ip route add 0.0.0.0 via 192.168.0.254
...
# see above, re-doing default route on host :/
ip netns exec $name ip route del 0.0.0.0 via 192.168.0.254
