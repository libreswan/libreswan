...
../../guestbin/wait-for.sh --match '#8: established Child SA' -- cat /tmp/pluto.log
../../guestbin/wait-for.sh --match '#9: established Child SA' -- cat /tmp/pluto.log
../../guestbin/wait-for.sh --match '#10: established Child SA' -- cat /tmp/pluto.log
../../guestbin/ping-once.sh --up -I 192.0.1.254 192.0.2.254
../../guestbin/ping-once.sh --up -I 192.0.100.254 192.0.200.254
../../guestbin/ping-once.sh --up -I 192.0.101.254 192.0.201.254
# wait for old, so that they are not in traffic status
