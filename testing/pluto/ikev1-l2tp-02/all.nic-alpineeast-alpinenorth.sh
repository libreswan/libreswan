...
nic# iptables -F
nic# # NAT
nic# iptables -t nat -A POSTROUTING --source 192.1.3.0/24 --destination 0.0.0.0/0 -j SNAT --to-source 192.1.2.254
nic# # make sure that we never acidentially let ESP or L2TP through.
nic# iptables -I FORWARD 1 --proto 50 -j DROP
nic# iptables -I FORWARD 2 --proto udp --dport 1701 -j DROP
nic# # Display the table, so we know it is correct.
nic# iptables -t nat -L -n
nic# iptables -L -n
nic# echo done
nic# : ==== end ====
