...
#define talloc_zero(ctx, type) (type *)_talloc_zero(ctx, sizeof(type), \
						    __location__)
#define talloc_realloc(ctx, ptr, size) _talloc_realloc(ctx, ptr, size, \
						       __location__)
...
#define talloc_zero_array(ctx, type, count) (type *)_talloc_zero_array(ctx, \
								       sizeof( \
									       type), count, \
								       __location__)
#define talloc_array(ctx, type, count) (type *)talloc_array_size(ctx, \
								 sizeof(type), \
								 count, \
								 __location__)
#define talloc_realloc_p(ctx, p, type, count) (type *)talloc_realloc_array(ctx, \
									   p, \
									   sizeof( \
										   type), count, \
									   __location__)
...
void talloc_set_name(const void *ptr, const char *fmt, ...) PRINTF_ATTRIBUTE(2,
									     3);
...
char *talloc_vasprintf(const void *t, const char *fmt,
		       va_list ap) PRINTF_ATTRIBUTE(2, 0);
char *talloc_asprintf(const void *t, const char *fmt, ...) PRINTF_ATTRIBUTE(2,
									    3);
