/*
 * EDDSA signature key generation, for libreswan
 *
...
static const struct curve curves[] = {
	{ "ed25519", SEC_OID_ED25519_PUBLIC_KEY, }
};
...
/* forwards */
static void eddsasigkey(SECOidTag curve, struct logger *logger);
...
int main(int argc, char *argv[])
{
	...
	eddsasigkey(curve, logger);
	exit(0);
}

/*
 * generate an EDDSA signature key
 */
static void eddsasigkey(SECOidTag curve, struct logger *logger)
{
	/*
	...
	SECKEYPublicKey *pubkey = NULL;
	{
		...
		SECItem *eddsaparams = SEC_ASN1EncodeItem(NULL, NULL, &oiddata->oid,
							  SEC_ObjectIDTemplate);

#if 0
		PK11SlotInfo *slot = PK11_GetBestSlot(CKM_EC_EDWARDS_KEY_PAIR_GEN,
						      lsw_nss_get_password_context(logger));
#else
		PK11SlotInfo *slot = lsw_nss_get_authenticated_slot(logger);
#endif
		if (slot == NULL) {
			llog_nss_error(ERROR_STREAM, logger, "PK11_GetBestSlot()");
			shutdown_nss();
			exit(1);
		}
		...
		PK11_FreeSlot(slot);
		SECITEM_FreeItem(eddsaparams, PR_TRUE/*also-free-item*/);
	}
	...
	fprintf(stderr, "Generated EDDSA key pair with CKAID %s was stored in the NSS database\n",
		hex_ckaid);
