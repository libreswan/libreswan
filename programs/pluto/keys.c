...
err_t ECDSA_signature_verify_nss(const struct ECDSA_public_key *k,
			       ...
			       enum notify_payload_hash_algorithms hash_algo UNUSED)
{
	DBGF(DBG_CRYPT, "ECDSA_signature_verify_nss");
	SECStatus retVal;
	...
	if (arena == NULL) {
		LSWLOG(buf) {
			lswlogs(buf, "NSS: failed to allocate area for EC key");
			lswlog_nss_error(buf);
		}
		PORT_SetError(SEC_ERROR_NO_MEMORY); /* XXX: why */
		return "10" "NSS error: Not enough memory to create arena";
	}

	/* copy k's public key value into the arena / publicKey */
	SECItem k_pub = same_chunk_as_secitem(k->pub, siBuffer);
	retVal = SECITEM_CopyItem(arena, &publicKey.publicValue, &k_pub);
	if (retVal != SECSuccess) {
		LSWLOG(buf) {
			lswlogs(buf, "NSS: failed to create EC key");
			lswlog_nss_error(buf);
		}
		PORT_FreeArena(arena, PR_FALSE);
...
#if 0
	LSWDBGP(DBG_CRYPT, buf) {
		lswlogs(buf, "nss_pub (should be same value as publicValue): ");
		lswlog_nss_secitem(buf, &publicKey.publicValue);
	}
#endif
	...
	if (retVal != SECSuccess) {
		LSWLOG(buf) {
			lswlogs(buf, "NSS: construction of ecParams failed");
			lswlog_nss_error(buf);
		}
		PORT_FreeArena(arena, PR_FALSE);
...
#if 0
	DBGF(DBG_CRYPT, "ecParams->fieldID.size %d", publicKey.ecParams.fieldID.size);
	DBGF(DBG_CRYPT, "ecParams->order.len %d", publicKey.ecParams.order.len);
	DBGF(DBG_CRYPT, "EC_GetPointSize(ecParams) %d", EC_GetPointSize(&publicKey.ecParams));
	LSWLOG(buf) {
		lswlogs(buf, "ecParams->order: ");
		lswlog_nss_secitem(buf, &publicKey.ecParams.order);
	}
#endif
	...
	SECItem hash = {
		.type = siBuffer,
		.data = PORT_ArenaZAlloc(arena, hash_len + 1),
		.len = hash_len + 1,
	};
	memcpy(hash.data, hash_val, hash_len);

	if (ECDSA_VerifyDigest(&publicKey, &signature, &hash) != SECSuccess) {
		DBGF(DBG_CRYPT, "ECDSA_VerifyDigest failed");
		PORT_FreeArena(arena, PR_FALSE);
		return "13" "NSS error: Not able to decrypt";
	}

	DBG(DBG_CRYPT,
	    DBG_dump("NSS ECDSA verify: decrypted sig: ", hash.data,
		     hash.len));
