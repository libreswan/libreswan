		...
		u_int16_t port /* host order */)
{
	...
	pexpect(!is_zero_cookie(rcookie));
			...
			struct state *st, bool initiator)
{
	...
	out_modify_previous_np(pd->pt, outs);

	/* output first NAT-OA */

	{
		const unsigned char *ip_val;
		size_t ip_len = addrbytesptr_read(ipinit, &ip_val);
		passert(ip_len != 0);
		...
		if (!out_struct(&natoa, pd, outs, &pbs) ||
		    !out_raw(ip_val, ip_len, &pbs, "NAT-OAi"))
			return FALSE;
	...
	{
		const unsigned char *ip_val;
		size_t ip_len = addrbytesptr_read(ipresp, &ip_val);
		passert(ip_len != 0);
		...
		if (!out_struct(&natoa, pd, outs, &pbs) ||
		    !out_raw(ip_val, ip_len, &pbs, "NAT-OAr"))
			return FALSE;
