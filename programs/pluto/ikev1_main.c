...
static void main_mode_sha1(struct state *st,
			   u_char *hash_val,            /* resulting bytes */
			   ...
			   const pb_stream *idpl)       /* ID payload, as PBS */
{
	...
	*hash_len = SHA1_DIGEST_SIZE;
	main_mode_hash_body(st, hashi, idpl, &ctx,
			    (void (*)(union hash_ctx *, const u_char *,
...
stf_status main_inI1_outR1(struct msg_digest *md)
{
	...
	if (c == NULL) {
		...
		if (c == NULL) {
			...
			DBG(DBG_CONTROL,
			    DBG_log(
				    "instantiating \"%s\" for initial Main Mode message received on %s:%u",
				    c->name,
				       ...
				       struct pluto_crypto_req *r)
{
	...
	if (!ship_nonce(&st->st_ni, r, &md->rbody,
			(cur_debugging &
			 IMPAIR_BUST_MI2) ? ISAKMP_NEXT_VID : ISAKMP_NEXT_NONE,
			"Ni"))
		return STF_INTERNAL_ERROR;
				...
				struct pluto_crypto_req *r)
{
	 ...
	 */
	{
		...
		DBG(DBG_CONTROLMORE,
		    DBG_log(
			    "main inI2_outR2: starting async DH calculation (group=%d)",
	...
	/* Notification Payload */
	{
		...
		if (spisize > 0) {
			if (!out_raw(spi, spisize, &not_pbs, "spi")) {
				libreswan_log(
					"failed to build notification for spisize=%d\n",
					(int)spisize);
				return;
			}
		}
	...
	*r_hash_start;  /* start of what is to be hashed */
	...
	if (isakmp_sa) {
		...
		while (ns != said) {
			...
			if (!out_struct(&isad, &isakmp_delete_desc, &r_hdr_pbs,
					&del_pbs) ||
			    !out_raw(&ns->spi, sizeof(ipsec_spi_t), &del_pbs,
				     "delete payload"))
				impossible();
			close_output_pbs(&del_pbs);
		}
