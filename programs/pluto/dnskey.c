...
void stop_adns(void)
{
	...
	if (adns_pid != 0) {
		...
		if (p == -1) {
			...
			libreswan_log("ADNS process terminated by signal %d",
				      (int)WTERMSIG(status));
			adns_pid = 0;
		} else {
			libreswan_log(
	...
	*e = '\0';
	if (*p == '@') {
		...
		if (strspn(p, " ") >= IDTOA_BUF)
			return builddiag(
				"malformed FQDN in TXT " our_TXT_attr_string ": ID too large for IDTOA_BUF");
		...
		if (ugh != NULL) {
			return builddiag(
				"malformed FQDN in TXT " our_TXT_attr_string ": %s",
				ugh);
		}
	} else {
		...
		if (ugh != NULL) {
			return builddiag(
				"malformed IP address in TXT " our_TXT_attr_string ": %s",
				ugh);
		}
				 ...
				 struct adns_continuation *const cr)
{
	...
	if (doit) {
		...
		if (gi.gw_key_present) {
			...
			if (ugh != NULL)
				return builddiag("malformed key data: %s",
						 ugh);
...
# ifdef USE_KEYRR
static err_t process_lwdnsq_key(char *str,
				enum dns_auth_level dns_auth_level,
				struct adns_continuation *const cr)
{
	...
	if (protocol == 4 &&                            /* IPSEC (RFC 2535 3.1.3) */
		...
		err_t ugh = ttodatav(rest, 0, 64, (char *)kb, sizeof(kb),
				     &kbc.len,
				     diag_space, sizeof(diag_space),
		...
		kbc.ptr = kb;
		TRY(add_public_key(&cr->id, dns_auth_level, PUBKEY_ALG_RSA,
				   &kbc,
				   &cr->keys_from_dns));
...
static err_t eat_name_helpfully(pb_stream *pbs, const char *context)
{
	...
	return ugh == NULL ? ugh :
	       builddiag("malformed name within DNS record of %s: %s", context,
			 ugh);
}
...
static err_t process_answer_section(pb_stream *pbs,
				    bool doit,          /* should we capture information? */
				    enum dns_auth_level *dns_auth_level,
				...
				u_char ans[], int anslen)
{
	...
	if (r != 0)
		return r <
		       (int)elemsof(rcode_text) ? rcode_text[r] :
		       "unknown rcode";
			    ...
			    const char *gwname)
{
	...
	DBG(DBG_CONTROL | DBG_DNS,
	    DBG_log("DNS query %lu for %s for %s (gw: %s)",
		    serial, typename, name_buf,
		    gwname));
	return NULL;
}
	...
	for (;; ) {
		if (buf_cur != buf_end) {
			...
			next_query->used = FALSE;
			{
				...
				DBG(DBG_DNS,
				    DBG_log("lwdnsq query: %.*s",
					    (int)(strlen(qbuf) - 1), qbuf));
				buf_cur = qbuf;
...
void handle_adns_answer(void)
{
	...
	while (buflen >= offsetof(struct adns_answer, ans) &&
		...
		if (buf.result == -1) {
			...
			case NO_DATA:
				ugh = builddiag("no %s record for %s",
						typename, name_buf);
				break;
			case HOST_NOT_FOUND:
	...
	for (;; ) {
		...
		*nlp++ = '\0';
		...
		if (ugh != NULL)
			libreswan_log(
				"failure processing lwdnsq output: %s; record: %s",
				ugh, buf);
