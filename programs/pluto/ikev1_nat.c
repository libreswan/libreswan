...
void ikev1_maybe_natify_initiator_endpoints(struct state *st, where_t where)
{
	/*
...
void v1_natify_initiator_endpoints(struct state *st, where_t where)
{
	 ...
	 */
	endpoint_buf b1, b2;
	ip_endpoint new_local_endpoint =
		set_endpoint_port(st->st_iface_endpoint->local_endpoint,
				  ip_hport(NAT_IKE_UDP_PORT), HERE);
	...
	ip_endpoint new_endpoint =
		set_endpoint_port(st->st_remote_endpoint,
				  ip_hport(NAT_IKE_UDP_PORT), HERE);
...
static bool ikev1_nat_update_family_mapp_state(struct state *st, void *data)
{
	...
	if (md != NULL) {
		...
		if (!endpoint_eq_endpoint(md->sender, st->st_remote_endpoint)) {
			...
			state_by_ike_spis(st->st_ike_version,
					  ...
					  NULL /* role */,
					  &st->st_ike_spis,
					  ikev1_nat_update_family_mapp_state,
					  &nfo,
					  __func__);
		}
