		...
		if (st == NULL) {
			...
			if (this->config->host.xauth.server &&
				...
				dbg("received isakmp_xchg_type %s; this is a%s%s%s%s in state %s. Reply with UNSUPPORTED_EXCHANGE_TYPE",
				    ...
				    st->st_connection->spd.this.config->client.modecfg_server ? " modecfgserver" : "",
				    st->st_connection->spd.this.config->client.modecfg_client ? " modecfgclient" : "",
				    st->st_state->name);
				return;
			}
		} else {
...
void complete_v1_state_transition(struct state *st, struct msg_digest *md, stf_status result)
{
	...
	case STF_OK:
	{
		...
		if (st->st_connection->local->config->host.xauth.client &&
		    ...
		    !st->st_connection->spd.this.config->client.modecfg_client &&
		    st->st_state->kind == STATE_XAUTH_I1)
		{
		...
		if (c->local->config->host.xauth.client &&
		    st->hidden_variables.st_xauth_client_done &&
		    !c->spd.this.config->client.modecfg_client &&
