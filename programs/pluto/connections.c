			       ...
			       struct connection *c)
{
	...
	if (NEVER_NEGOTIATE(wm->policy)) {
		...
		dbg("AA_2020 %s %d %s sa_clone_id %u sa_clones %u", __func__, __LINE__, c->name, c->sa_clone_id, c->sa_clones);
	...
	if (c->sa_reqid == 0) {
		if (c->sa_clones == 0) {
			...
			/* AA_2020 some other way to distribute than this ??? */
			c->sa_reqid = c->spd.reqid = IPSEC_MANUAL_REQID_MAX - 1;
			DBG_log("AA_2020 %s %d %s harcode reqid %u", __func__, __LINE__, c->name, c->spd.reqid);
		...
		DBG_log("AA_2020 %s %d %s use configured reqid %u", __func__, __LINE__, c->name,
				c->spd.reqid);
	}
