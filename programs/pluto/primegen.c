			...
			MPI g, MPI **ret_factors )
{
	...
	prime =
		mpi_alloc( (pbits + BITS_PER_MPI_LIMB -
			    1) /  BITS_PER_MPI_LIMB );
...
next_try:
	...
	if ( DBG_CIPHER ) {
		...
		log_debug("bit sizes: prime=%u, q=%u", mpi_get_nbits(
				  prime), mpi_get_nbits(q) );
		if ( mode == 1 )
			fprintf(stderr, ", q0=%u", mpi_get_nbits(q_factor) );
		for (i = 0; i < n; i++ )
			fprintf(stderr, ", p%d=%u", i, mpi_get_nbits(
					factors[i]) );
		progress('\n');
	}
...
static MPI gen_prime( unsigned nbits, int secret, int randomlevel )
{
	unsigned nlimbs;
...
static int is_prime( MPI n, unsigned steps, int *count )
{
	...
	for (i = 0; i < steps; i++ ) {
		...
		if ( !i ) {
			...
			assert( mpi_cmp( x, nminus1 ) < 0 && mpi_cmp_ui( x,
									 1 ) >
				0 );
		}
		mpi_powm( y, x, q, n);
