...
static struct connection_client connection_spd_client(const struct spd_end *spd)
{
	struct connection_client client = {
		...
		.is_addresspool = (len(spd->child->config->addresspools) > 0),
	};
	return client;
}
							 ...
							 const ip_selector *this_selector)
{
	struct connection_client client = {
		...
		.is_addresspool = (len(this->child.config->addresspools) > 0),
	};
	return client;
}
		    ...
		    const char *separator)
{
	...
	if (selector_is_all(this->client)) {
		...
		if (len(this->child->config->addresspools) > 0) {
			/*
...
#undef COMBO
	...
	if (c->config->ike_version == IKEv2) {
		SHOW_JAMBUF(s, buf) {
			...
			jam_authby_sighash_policy(buf, c->local->config->host.authby);
			jam_string(buf, ";");
		}
	}
