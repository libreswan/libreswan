...
#undef MAX_DISPLAY_BYTES
	...
	bool ok = (result >= 0 && (size_t)result < blen);

	char jbuf[2048+1];
	jambuf_t jambuf = ARRAY_AS_JAMBUF(jbuf);
	jam_common_shell_out(&jambuf, c, sr, st, inbytes, outbytes);
	pexpect(ok == jambuf_ok(&jambuf));
	if (ok && !streq(buf, jbuf)) {
		PEXPECT_LOG("jambuf shell string <<%s>>", jbuf);
		PEXPECT_LOG("printf shell string <<%s>>", buf);
		for (int i = 0; buf[i] || jbuf[i]; i++) {
			if (buf[i] != jbuf[i]) {
				PEXPECT_LOG("%d", i);
				PEXPECT_LOG("jambuf <<%s>>", jbuf+i);
				PEXPECT_LOG("printf <<%s>>", buf+i);
				break;
			}
		}
	}
