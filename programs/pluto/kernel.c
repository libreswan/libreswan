...
init_kernel(void)
{
	...
	if (stat("/proc/net/pfkey", &buf) == 0) {
	    ...
	    libreswan_log("Using Linux XFRM/NETKEY IPsec interface code on %s"
			 , kversion);
	    kernel_ops = &netkey_kernel_ops;
	    break;
	} else
	    libreswan_log("No Kernel XFRM/NETKEY interface detected");
	/* FALL THROUGH */
#endif
...
install_ipsec_sa(struct state *st, bool inbound_also USED_BY_KLIPS)
{
   ...
   if (st->st_connection->remotepeertype == CISCO) {
	...
	if(!st->st_connection->newest_ipsec_sa && st->spd->this.xauth_client) {
...
delete_ipsec_sa(struct state *st USED_BY_KLIPS, bool inbound_only USED_BY_KLIPS)
{
	...
	if (st->st_connection->remotepeertype == CISCO && st->spd->this.xauth_client &&
