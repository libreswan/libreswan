...
$(eval $(call error_replaced, 5.4, LIBEXECDIR, IPSEC_EXECDIR))
...
# IPSEC_EXECDIR is where libreswan's sub-commands get put.  the
# "ipsec" command will look for them in this directory when it is run.
#
# It should really be:
#
#   LIBEXECDIR?=$(PREFIX)/libexec
#   IPSEC_EXECDIR?=$(LIBEXECDIR)/ipsec
#
# but a SNAFU - LIBEXECDIR=.../ipsec - only fixed in 5.4 means that
# will have to wait

IPSEC_EXECDIR ?= $(PREFIX)/libexec/ipsec
TRANSFORMS += -e 's:@@IPSEC_EXECDIR@@:$(IPSEC_EXECDIR):g'
USERLAND_CFLAGS += -DIPSEC_EXECDIR=\"$(IPSEC_EXECDIR)\"
...
TRANSFORMS += -e 's:@@DEFAULT_DNSSEC_ROOTKEY_FILE@@:$(DEFAULT_DNSSEC_ROOTKEY_FILE):g'
else ifeq ($(USE_UNBOUND),true)
$(error USE_UNBOUND requires DEFAULT_DNSSEC_ROOTKEY_FILE)
else
TRANSFORMS += -e 's:@@DEFAULT_DNSSEC_ROOTKEY_FILE@@:not-applicable:g'
endif
