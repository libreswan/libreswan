...
struct ipsec_conf *parser_load_conf(const char *file, struct logger *logger,
				    bool setuponly, unsigned verbosity);
struct ipsec_conf *parser_argv_conf(const char *name, char *argv[], int start, struct logger *logger);

void parser_freeany_config_parsed(struct ipsec_conf **cfg);
