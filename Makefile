...
MAIN_RPM_VERSION = $(shell make --no-print-directory showversion | sed "s/-.*//")
MAIN_RPM_PREVER = $(shell make --no-print-directory showversion | sed -e  "s/^.[^-]*-\([^-]*\)-\(.*\)/rc\1_\2/" -e "s/-/_/g" -e "s/\//_/g")
