				       ...
				       struct pfkey_extracted_data* extr)
{
	...
	if (extensions[K_SADB_EXT_SPIRANGE]) {
		minspi =
			((struct sadb_spirange *)extensions[K_SADB_EXT_SPIRANGE
			 ])->
			sadb_spirange_min;
		maxspi =
			((struct sadb_spirange *)extensions[K_SADB_EXT_SPIRANGE
	...
	sa_len =
		KLIPS_SATOT(debug_pfkey, &extr->ips->ips_said, 0, sa,
			    sizeof(sa));
...
#endif
	if (ip_chk_addr((unsigned long)extr->ips->ips_said.dst.u.v4.sin_addr.
			s_addr) == IS_MYADDR)
		extr->ips->ips_flags |= EMT_INBOUND;
	...
	if (!(pfkey_safe_build(error =
	      ...
	      pfkey_safe_build(error =
				       pfkey_sa_build(&extensions_reply[
							      K_SADB_EXT_SA],
						      K_SADB_EXT_SA,
	...
	if ((error =
		     pfkey_msg_build(&pfkey_reply, extensions_reply,
				     EXT_BITS_OUT))) {
		KLIPS_PRINT(debug_pfkey, "klips_debug:pfkey_getspi_parse: "
	...
	for (pfkey_socketsp = pfkey_open_sockets;
		...
		if ((error =
			     pfkey_upmsg(pfkey_socketsp->socketp,
					 pfkey_reply))) {
			KLIPS_PRINT(debug_pfkey, "klips_debug:pfkey_getspi_parse: "
	...
	if (extr->ips->ips_natt_sport || extr->ips->ips_natt_dport) {
		...
		if (extr->ips->ips_natt_sport) {
			...
			if (ipsq->ips_addr_s->sa_family == AF_INET)
				((struct sockaddr_in *)(ipsq->ips_addr_s))->
				sin_port = htons(extr->ips->ips_natt_sport);
		}

		if (extr->ips->ips_natt_dport) {
			ipsq->ips_natt_dport = extr->ips->ips_natt_dport;
			if (ipsq->ips_addr_d->sa_family == AF_INET)
				((struct sockaddr_in *)(ipsq->ips_addr_d))->
				sin_port = htons(extr->ips->ips_natt_dport);
		}
	...
	if (!(pfkey_safe_build(error =
		  ...
		  pfkey_safe_build(error =
					   pfkey_sens_build(&extensions_reply[
							    ...
							    ips_sens_sens_len,
							    extr->ips->
							    ips_sens_sens_bitmap,
							    extr->ips->
							    ips_sens_integ_level,
							    extr->ips->
							    ips_sens_integ_len,
							    extr->ips->
							    ips_sens_integ_bitmap),
				   extensions_reply) : 1)
#endif
	      )) {
				    ...
				    struct pfkey_extracted_data* extr)
{
	...
	if (!(pfkey_safe_build(error =
	      ...
	      pfkey_safe_build(error =
			       ...
			       extensions_reply) &&
	      pfkey_safe_build(error =
				       pfkey_saref_build(&extensions_reply[
								 K_SADB_X_EXT_SAREF
							 ],
							 extr->ips->ips_ref,
							 extr->ips->ips_refhim),
				       ...
				       struct pfkey_extracted_data* extr)
{
	...
	if (!(pfkey_safe_build(error =
			       ...
			       extensions_reply) &&
	      pfkey_safe_build(error =
				       pfkey_sa_builds(&extensions_reply[
							       K_SADB_EXT_SA],
						       sab),
			       extensions_reply) &&
	      pfkey_safe_build(error =
				    ...
				    struct pfkey_extracted_data* extr)
{
	...
	if (!(pfkey_safe_build(error =
		  ...
		  pfkey_safe_build(error =
					   pfkey_key_build(&extensions_reply[
								   K_SADB_EXT_KEY_AUTH
							   ],
							   K_SADB_EXT_KEY_AUTH,
							   extr->ips->
...
int pfkey_register_reply(int satype, struct sadb_msg *sadb_msg)
{
	...
	struct sadb_alg *alg_a = NULL, *alg_e = NULL, *alg_ap = NULL, *alg_ep =
		NULL;
	int error = 0;
	...
	if (alg_num_a) {
		...
		if ((alg_a =
			     kmalloc(alg_num_a * sizeof(struct sadb_alg),
				     GFP_ATOMIC) ) == NULL) {
			KLIPS_PRINT(debug_pfkey,
				    "klips_debug:pfkey_register_reply: "
	...
	if (alg_num_e) {
		KLIPS_PRINT(debug_pfkey,
			    ...
			    (unsigned long) (alg_num_e *
					     sizeof(struct sadb_alg)));
		if ((alg_e =
			     kmalloc(alg_num_e * sizeof(struct sadb_alg),
				     GFP_ATOMIC) ) == NULL) {
			KLIPS_PRINT(debug_pfkey,
				    "klips_debug:pfkey_register_reply: "
	...
	while (pfkey_supported_listp) {
		if (alg_num_a) {
			if (pfkey_supported_listp->supportedp->ias_exttype ==
			    K_SADB_EXT_SUPPORTED_AUTH) {
				alg_ap->sadb_alg_id =
					pfkey_supported_listp->supportedp->
					ias_id;
	...
	if (!(pfkey_safe_build(error =
	      ...
	      (alg_num_a ? pfkey_safe_build(error =
						    pfkey_supported_build(&
									  ...
									  alg_a),
					    extensions_reply) : 1) &&
	      (alg_num_e ? pfkey_safe_build(error =
						    pfkey_supported_build(&
									  extensions_reply
									  [
										  K_SADB_EXT_SUPPORTED_ENCRYPT
					...
					struct pfkey_extracted_data* extr)
{
	...
	if (!(pfkey_safe_build(error =
	      ...
	      (extr->ips2 ?
	       (pfkey_safe_build(error =
					 pfkey_x_satype_build(&extensions_reply
							      [
...
/*	if(ip_chk_addr((unsigned long)extr->ips->ips_said.dst.u.v4.sin_addr.s_addr) == IS_MYADDR) */
		...
		if (extr->ips->ips_flags & SADB_X_SAFLAGS_REPLACEFLOW) {
			...
			if ((error =
				     ipsec_breakroute(&(extr->eroute->er_eaddr),
					  ...
					  struct pfkey_extracted_data* extr)
{
	...
	error = pfkey_msg_hdr_build(&extensions_reply[0],
				    ...
				    0,
				    ((struct sadb_msg*)extensions[
					     K_SADB_EXT_RESERVED])->sadb_msg_seq,
				    ((struct sadb_msg*)extensions[
...
int pfkey_expire(struct ipsec_sa *ipsp, int hard)
{
	...
	if (!(pfkey_safe_build(error = pfkey_msg_hdr_build(&extensions[0],
			       ...
			       extensions) &&
	      pfkey_safe_build(error =
				       pfkey_sa_build(&extensions[K_SADB_EXT_SA
						      ],
						      K_SADB_EXT_SA,
						      ipsp->ips_said.spi,
						      ipsp->ips_replaywin,
			    ...
			    __u16 sport)
{
	...
	if (!(pfkey_safe_build
		      (error =
			      pfkey_msg_hdr_build(&extensions[0],
	      ...
	      && pfkey_safe_build
		      (error =
			      pfkey_x_nat_t_port_build(&extensions[
					 ...
					 int err)
{
	...
	if ((error =
		     pfkey_msg_build(&pfkey_reply, extensions,
				     EXT_BITS_OUT))) {
		KLIPS_PRINT(debug_pfkey, "klips_debug:pfkey_expire: "
			    "failed to build the expire message\n");
		SENDERR(-error);
...
DEBUG_NO_STATIC int(*msg_parsers[K_SADB_MAX + 1])(struct sock *sk,
						  struct sadb_ext *extensions[],
...
int pfkey_msg_interp(struct sock *sk, struct sadb_msg *pfkey_msg)
{
	...
	if ((error =
		     pfkey_msg_parse(pfkey_msg, NULL, extensions,
				     EXT_BITS_IN))) {
		KLIPS_PRINT(debug_pfkey,
			    "klips_debug:pfkey_msg_interp: "
	...
	if ((error =
		     msg_parsers[pfkey_msg->sadb_msg_type](sk, extensions,
							   &extr))) {
		KLIPS_PRINT(debug_pfkey,
			    "klips_debug:pfkey_msg_interp: "
			    "message parsing failed with error %d.\n",
