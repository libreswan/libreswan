	...
	if (o_verbose) {
		...
		if (h_errno) {                  /* if host-lookup variety of error ... */
			if (h_errno > 4)        /* oh no you don't, either */
				fprintf(stderr, "preposterous h_errno: %d",
					h_errno);
			else
...
#endif
		holler("DNS fwd/rev mismatch: %s != %s", poop->name,
		       hp->h_name);
		return 1;
	}
	return 0;
...
/* numeric or not, failure to look up a PTR is *not* considered fatal */
		if (!hostent) {
			...
			strncpy(poop->name, hostent->h_name, MAXHOSTNAMELEN -
				2);
...
/* If you want to screw with RCVBUF/SNDBUF, do it here.  Liudvikas Bukys at
   ...
   integers set in main() getopt loop, and check for rr == 0 afterward. */
	...
	if (o_saref != INVALID_SAREF) {
		...
		rr = setsockopt(nnetfd, IPPROTO_IP, IP_IPSEC_BINDREF, &arg,
				sizeof(arg));
		if (rr == -1)
...
/* If a request was made to bind this connection to a SAref, do so now before
   we connect and exchange any more packets. */
		...
		if (o_saref != INVALID_SAREF) {
			long arg = o_saref;
			rr = setsockopt(nnetfd, IPPROTO_IP, IP_IPSEC_BINDREF,
					&arg, sizeof(arg));
			if (rr == -1)
				bail("failed setting IPsec SAref");
		}
...
/* do a full lookup [since everything else goes through the same mill],
   ...
   be useful, so we'll still pass o_nflag here instead of forcing numeric.  */
		...
		case 'w':               /* wait time */
			...
			timer1 =
				(struct timeval *) Hmalloc(sizeof(struct
								  timeval));
			timer2 =
				(struct timeval *) Hmalloc(sizeof(struct
								  timeval));
...
/* Now start connecting to these things.  curport is already preloaded. */
		while (loport <= curport) {
			...
			Debug(("netfd %d from port %d to port %d", netfd,
			       ourport, curport))
			if (netfd > 0)
